...
/**
 * Schema inicial do template: `app_tenants`, `samples`, `users`, `audit_logs`.
 *
 ...
 * DROP/CREATE de indice sem motivo.
 *
 * Todo unique que convive com soft delete e um **indice parcial**
 * (`WHERE "deletedAt" IS NULL`) — `app_tenants.document`,
 * `users.supertokensUserId` e `users (tenantId, email)`. Com um unique comum a
 * linha soft-deletada continua ocupando o valor e bloqueia para sempre o
 * recadastro do mesmo documento/e-mail.
 */
